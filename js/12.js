...
//! рабочая финальная версия простого калькулятора и порфесионального
...
(function () {
  ...
  function recalculate() {
    // Получаем значение в USD из инпута калькулятора
    const usdValue = totalAsValue;
    console.log("usdValue ", usdValue);

    if (!isNaN(usdValue)) {
      // Конвертируем в текущую валюту
      const convertedValue = (usdValue * currentCurrencyRate).toFixed(0);
      console.log("Конвертированное значение: ", convertedValue);

      // Обновляем значение на экране с нужным символом валюты
      totalAs.textContent = `${convertedValue} ${symbolValut}`;
  ...
  function updateCoefficient() {
    if (selectedFuel) {
      ...
      exciseDuty.textContent = `${exciseDutyValue}  ${symbolValut}`;
    }
    getAsTotal(); // Пересчитываем итоговую сумму
  }
  ...
  function getCurrencySymbol() {
    switch (currencyId) {
      case "usd":
        return (symbolValut = "$");
      case "pl":
        return (symbolValut = "zł"); // Символ для PLN (Польский злотый)
      case "eur":
        return (symbolValut = "€"); // Символ для EUR (Евро)
      default:
