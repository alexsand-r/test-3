"use strict";
//! в этом коде не считает сума итого сборов на професиональном калькуляторе
...
(function () {
  ...
  function updateCoefficient() {
    if (selectedFuel) {
      ...
      // console.log("знак валюты - ", symbolValut);
    }
  ...
  function recalculate() {
    ...
    if (!isNaN(usdValue)) {
      ...
      totalAs.textContent = `${convertedValue} ${symbolValut}`; // todo-----!
      // vat.textContent = `${vatValue} ${symbolValut}`; // todo----!
      console.log("выбранная валюта в функции recalculate() ", symbolValut);
    }
  }
