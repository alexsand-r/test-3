"use strict";
/*
Этот код динамически обновляет список городов и стоимость доставки в зависимости от выбранной страны и города. При выборе страны из выпадающего списка отображается соответствующий список городов. Затем, при выборе конкретного города, обновляется цена доставки, которая отображается на странице.
Скрипт также задает страну по умолчанию при загрузке страницы(Польша) и сразу показывает список её городов и стоимость доставки для первого города в списке.
  */
(function () {
...
/* 
Получение начальных данных: Извлекаются элементы страницы, такие как инпут стоимости автомобиля, кнопки для увеличения и уменьшения цены, а также элементы для отображения промежуточных и итоговых расчетов.

Функция обновления цены: При изменении значения инпута или нажатии кнопок "+" или "-" обновляется цена автомобиля, проверяется, что она не меньше нуля, и отображается на странице.

Расчеты сборов и налога:

updateAuctionFee(): рассчитывает аукционный сбор (5% от стоимости автомобиля).
updateTaxAmount(): рассчитывает налог на основе стоимости автомобиля, аукционного сбора, стоимости доставки, и других услуг.
updateTotalPrice(): вычисляет итоговую стоимость, включая все дополнительные сборы и налог.
Наблюдатель за стоимостью доставки: Использует MutationObserver для отслеживания изменений стоимости доставки. При изменении стоимости доставки пересчитываются все значения.

Обработчики событий:

Обновляют расчеты при вводе значения стоимости в инпут вручную.
Пересчитывают значения при нажатии на кнопки "+" и "-".
Инициализация: При загрузке страницы устанавливаются начальные значения и выполняются все расчеты для отображения актуальной информации.*/
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (inputElement) {
    ...
    function updateTaxAmount() {
      const auctionFee = document.getElementById("auction-fee");
      const unloadingForwarding = document.getElementById(
