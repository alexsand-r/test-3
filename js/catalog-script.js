...
const arrayYearSlider = [2015, currentYear];
...
function hideSearchBlocks(btnCross) {
  blockSearch.forEach((blockElement) => {
...
const noneBreadcrumbArr = [];

function closeDropdownOnClickOutside(event, noneBreadcrumb, dataDropdown) {
  if (!noneBreadcrumb.contains(event.target) && event.target !== dataDropdown) {
    noneBreadcrumb.classList.add("none-breadcrumb");
    document.removeEventListener("click", closeDropdownOnClickOutside);
  }
}

function openDropdown(event, dataDropdown, dropdownId) {
  const noneBreadcrumb = document.getElementById(dropdownId);
  if (noneBreadcrumb) {
    event.stopPropagation();
    noneBreadcrumb.classList.toggle("none-breadcrumb");
    if (!noneBreadcrumbArr.includes(noneBreadcrumb)) {
      noneBreadcrumbArr.push(noneBreadcrumb);
      document.addEventListener("click", (event) => {
        closeDropdownOnClickOutside(event, noneBreadcrumb, dataDropdown);
      });
    }
  }
}
const default_open_filters = document.querySelectorAll(".default_open_filters");
if (default_open_filters && window.innerWidth > 767) {
  default_open_filters.forEach((item) => {
    item.classList.remove("hidden");
  });
}
//*-------------------------------------------------------------------
// в файле -  catalog-script.js в самом низу есть код
// const default_open_filters = document.querySelectorAll(".default_open_filters");
// if (default_open_filters && window.innerWidth > 767) {
//   default_open_filters.forEach((item) => {
//     item.classList.remove("hidden");
//   });
// }
// который проверяет наличие класса default_open_filters в блоке
<div class="mt-5 h-[120px] overflow-auto pr-3 default_open_filters hidden"></div>;
// клас default_open_filters дописан руками , так вот при наличии этого класа удаляется класс хиден
// ну типа при расширении меньше 767  блок в закрытом состоянии а при расширении больше 767
// блоки в отрытом состоянии. Может у вас и была такая задумка, не знаю. Попробуй убери код дж из файла
// catalog-script.js и посмотри как будет работать
