...
function updateCustomsDuty() {
  if (isNaN(sumaAvto) || sumaAvto === 0) {
    ...
    customsDuty = Math.round((sumaAvto + countryOfDelivery) * PRECENT_CUSTOMS);
  } else if (prof === true) {
    customsDuty = Math.round(
      (sumaAvto + startShippingCost + startAuctionCommission) * PRECENT_CUSTOMS
...
function updateVatValue() {
  ...
  if (isNaN(sumaAvto) || sumaAvto === 0) {
    ...
    if (selectPrecentVat == 0.23) {
      vatValue = Math.round(
        (sumaAvto + startShippingCost + startAuctionCommission + customsDuty) *
          VAT_PERCENT_POLAND
      );
    } else {
      vatValue = Math.round(
